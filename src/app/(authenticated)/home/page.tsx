...
import { Home, Send, Loader2, AlertCircle, PanelLeft, MessageSquare, CalendarCheck, ArrowDown, Plus } from 'lucide-react'
...
function HomeContent() {
  ...
  const chat = useConversationChat({
    sseOrganization: 'home',
    chatApiEndpoint: '/api/home-planner',
    basePath: '/home',
...
function ChatPanel({ chat, onOpenConversations }: ChatPanelProps) {
  return (
    <div className="flex-1 flex flex-col overflow-hidden min-w-0">
      ...
      <div className="md:hidden flex items-center justify-between px-3 py-2 border-b bg-background">
        <button
      ...
      {chat.messages.length === 0 ? (
        <div className="flex-1 flex items-center justify-center p-4 sm:p-6 text-muted-foreground">
          <div className="text-center max-w-lg px-2">
            <Home className="h-12 w-12 mx-auto mb-4 text-primary/50" />
          ...
          {chat.userScrolledUp && (
            <div className="sticky bottom-2 flex justify-center pointer-events-none">
              <Button
                size="icon"
                variant="secondary"
                className="pointer-events-auto h-8 w-8 rounded-full shadow-lg"
